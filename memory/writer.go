...
// DPAFormatStringWriter abstracts writing data to memory using the direct
// parameter access (DPA) feature of a format string.
//
// Strings generated by this writer are generally of the form:
//	%<some-number>c<delimiter>%<dpa-argument-number>$n<delimiter><padding><address-to-write-to>
//
// For example:
//	%100c|%8$n|AAAAAAAA<raw-address-bytes>
// ... would overwrite the lower four bytes of memory pointed to by the DPA
// argument number, which happens to be the "raw-address-bytes" with the
// value 101 (base 10). The reason it writes the number 101 instead of 100
// is because the "%8$n" includes the delimiter character ('|') in the count.
//
// This couples the "%<width>c" and the '%n' format specifiers with the DPA
// argument number, which points to a position within the format string.
// The first specifier forces the format function to write some number
// of characters to the resulting string. The '%n' then takes the number of
// characters written to the format string, and writes that number to
// a memory address pointed to by the DPA argument number.
//
// The number of bytes overwritten at the target address is adjusted using
// the 'h' specifier. Just the '%n' alone will overwrite the lower four bytes
// (32-bits) of memory with the new number. Adding a single 'h' ("%hn") will
// overwrite the two lower bytes at the address. Adding a second 'h' ("%hhn")
// will overwrite the lowest byte at the address.
